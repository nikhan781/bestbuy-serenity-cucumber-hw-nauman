Feature: Testing different request on the best buy application
  ...
  Scenario: Check if the best buy applications product api can be accessed by GET request
    When User sends a GET request to products endpoint
    Then User must get back a valid status code 200

  Scenario: Check if the best buy applications product api can be accessed by POST request
  ...
  Scenario: Check if the best buy applications product api can be accessed by PUT request
  ...
  Scenario: Check if the best buy applications product api can be accessed by DELETE request
